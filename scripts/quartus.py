...
def update_qsf_file(project_path: Path, top_level: str, mode: str):
...
def main():
    ...
    if args.target:
        ...
        dep_files += update_qsf_file(project_path, "target", mode="component")
    else:
        print(f"\n[Normal Mode] Project: {args.project}\n")
        dep_files += update_qsf_file(project_path, module_name, mode="normal")
    
    if args.compile:
        dep_files.append(generate_sdc(module_name, project_path / f"{project_name}.sdc", args.freq))
    ...
    if args.compile:
        ...
        report = generate_report(project_name, project_path, module_name)
